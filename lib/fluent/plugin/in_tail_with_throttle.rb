...
module Fluent::Plugin
  class TailInput < Fluent::Plugin::Input
    ...
    def construct_watcher(target_info, group_watcher)
      pe = nil
      if @pf
        pe = @pf[target_info]
      ...
      begin
        tw = setup_watcher(target_info, pe, group_watcher)
    ...
    def update_watcher(target_info, pe, group_watcher)
      ...
      if @follow_inodes
        ...
        if new_position_entry.read_inode == 0
          ...
          @tails[new_target_info] = setup_watcher(new_target_info, new_position_entry, group_watcher)
    ...
    def io_handler(watcher, path, group_watcher)
    ...
    class TailWatcher
      def initialize(target_info, pe, log, read_from_head, follow_inodes, update_watcher, line_buffer_timer_flusher, group_watcher, io_handler_build)
          ...
          if watcher_needs_update
            if @follow_inodes
              ...
              if stat
                ...
                @update_watcher.call(target_info, @pe, @group_watcher)
              end
            else
      ...
      def io_handler
        @io_handler_build.call(self, @path, @group_watcher)
      end
      ...
      class IOHandler
        ...
        def initialize(watcher, path:, read_lines_limit:, read_bytes_limit_per_second:, log:, open_on_every_update:, from_encoding: nil, encoding: nil, group_watcher:, &receive_lines)
          @watcher = watcher
          @path = path
          @read_lines_limit = read_lines_limit
          @read_bytes_limit_per_second = read_bytes_limit_per_second
